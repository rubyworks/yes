...
* It is also difficult to correlate entries, e.g. "node A can't be present if there is a node B".
...
Cone on folks! Let's get YAML up to snuff. A good Schema could really
...
so poor Ruby OSS developers can eat :)
